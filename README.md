...
- 迭代接口（以集合容器的迭代器为例）：
	- 获取容器的迭代器
	```
	SetIterator it = set_iterator(Container_p container, int reverse);	// reverse == 0: 正向迭代; 非0值: 反向迭代，部分容器可能不支持反向迭代，对这类容器reserve值无意义
	```
	- 迭代读取元素
	```
	Element e;
	while ((e = set_it_next(it))) {
		// do something...
		element_destroy(e);
	}
	```
	...
	```
	Element e = set_it_next(it);
	set_it_remove();			// 删除容器中的元素e，迭代位置回到e的前驱，如果e是第一个元素则回到初始位置
	element_destroy(e);
	```
	- 迭代器销毁
	```
	set_it_destroy(it);			// 不再使用后必须及时销毁迭代器
	```
