 ...
 *
 * @return
 *	池中有元素返回0，池为空或容器无效或容器不是池时返回1
 */
extern int pool_isempty(Container pool);

/**
 * @brief 获取池的使用率，使用百分率数值
 *
 * @param pool
 *	池容器
 *
 * @return
 *	池的使用率，即(元素数量/池容量)×100.0，容器无效或容器不是池时返回0.0
 */
...
/**
 * @brief 缩小池的容量，缩小到当前最后一个非空节点后剩余10个空闲节点
...
/**
 * @brief 清空池中所有元素并销毁所分配的内存
 *
 * @param pool
 *	要清空的池
 *
 * @return
 *	清空成功返回被清空的元素数量，清空失败返回-1
 */
